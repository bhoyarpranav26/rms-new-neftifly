...
const OrderTrackingPopup: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const justPaid = sessionStorage.getItem('paymentCompleted') === 'true'

    if (justPaid) {
      // Find the most recent order and show it
      const latestOrder = orders[orders.length - 1] // Assuming orders are in chronological order
      if (latestOrder) {
