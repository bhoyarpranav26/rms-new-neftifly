  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        },
        {
          ...
          # Inject sensitive values from Secrets Manager so they are not stored in plain text in the task definition
          secrets = [
            {
              name      = "DB_PASSWORD"
...
resource "aws_ecs_service" "app" {
  ...
  network_configuration {
    # Run tasks in the private subnets (no public IP). Ensure NAT gateway exists if tasks need outbound internet access
