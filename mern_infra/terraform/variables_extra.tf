...
// Optional variable to allow providing the email secret value at plan time.
// Prefer setting the secret value manually in AWS or via CLI rather than embedding it here.
variable "email_secret_value" {
  type    = string
  default = ""
  description = "Optional JSON string to initialize the email secret (e.g. '{\"SENDGRID_API_KEY\":\"<key>\"}'). Leave empty to set the secret via AWS Console or CLI."
}
